...
匿名クローンでリポジトリに到達できるため、GitHub連携無しでも
`git push`のたびに自動デプロイが動く。
...
- **マイグレーション追加時**: 新しいマイグレーションをコミットしてpushすれば、
  リポジトリが公開のためRenderの自動デプロイが走り、次回ビルドの
  `prisma migrate deploy`で自動適用される。手動デプロイしたい場合は
  `POST /v1/services/{id}/deploys`を直接呼ぶこともできる。
