...
- **樹木台帳**: 樹木・診断・点検・作業履歴・点検/作業予定・植替え・苦情・委託事業者の8エンティティをCRUD。写真添付(被害部写真・点検写真・作業前後写真)、診断カルテ(PDF)添付
- **地図**: クラスタリング表示、健全度による色分け、ホバーで樹木番号表示、クリックで詳細へ、ドラッグで位置修正、空き地クリック/現在地ボタンでの新規登録
- **点検・作業予定管理**: 定期点検・作業の「予定」(実施日より前の計画)を登録・進捗管理。実施済みの点検/作業履歴とは別テーブルで管理
- **カレンダー**: 苦情・予定・作業実績を月表示グリッドで横断的に確認。日付をクリックするとその日の一覧を表示
- **絞込検索**: 全エンティティの一覧画面で、項目ごとの絞込検索(選択式・部分一致)に対応。期間(開始日/終了日)指定検索とあわせて利用可能
...
- **入出力**: CSV/Excel出力(全エンティティ)、期間指定の作業履歴PDF出力、期間指定検索、指定期間の樹木数量集計
...
- [x] 定期点検や作業予定を登録できること(機能要件#24)が未実装だった(Inspection/WorkHistoryは実施済みの実績記録のみで、未来の予定を表すテーブルが無かった)→ 予定種別(点検予定/作業予定)・進捗状況(予定/進行中/完了/中止)を持つScheduleテーブルを新設
- [x] 陳情・作業の予定・進捗状況・履歴をカレンダーで確認できること(機能要件#13)が未実装だった(一覧表示のみで、月表示等のカレンダーUIが無かった。また苦情一覧はナビゲーションメニューから到達する導線自体が無かった)→ 苦情・予定・作業履歴を日付軸で集約するAPIとカレンダー画面を追加し、あわせて苦情一覧へのナビゲーションリンクも追加
- [x] 台帳の各項目ごとに絞込検索ができること(機能要件#19)が未実装だった(樹木一覧の樹木番号による部分一致検索が1つあるのみだった)→ 汎用CRUDルーターにホワイトリスト式の項目別絞込を追加し、全エンティティの一覧画面に絞込UIを追加
